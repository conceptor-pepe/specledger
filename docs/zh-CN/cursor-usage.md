...
这份文档用于约束团队在 Cursor 中如何稳定触发 SpecLedger 工作流。
...
所有改动都进入同一条 SpecLedger 工作流，但按风险分层：
...
- `.cursor/commands/specledger-run.md`
- `.cursor/commands/specld-run.md`
- `.cursor/commands/specledger-start.md`
- `.cursor/commands/specledger-continue.md`
- `.cursor/commands/specledger-approve.md`

推荐通过以下方式安装：

```bash
specledger/install/init.sh --target /path/to/repo --profile backend-brownfield --tool cursor
```
...
```text
这是一个新特性，请严格按 specld:run 进入 SpecLedger 工作流。
...
```text
这是一个复杂修改，请按 specld:run 进入 SpecLedger 工作流。
...
```text
这是一个简单修改，也请按 specld:run 进入 SpecLedger 工作流。
...
```text
按 specld:run 进入 SpecLedger 工作流。
```
