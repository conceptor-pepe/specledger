...
usage() {
  cat <<'EOF'
Usage:
  scripts/specledger-driver.sh start <change-name> [--date YYYY-MM-DD]
  scripts/specledger-driver.sh status <change-dir>
  scripts/specledger-driver.sh exec <change-dir>
  scripts/specledger-driver.sh approve <change-dir>
  scripts/specledger-driver.sh next <change-dir>
EOF
}
...
case "$subcmd" in
  start)
    ...
    change_name="$1"
    shift
    "$script_dir/specledger-start.sh" "$change_name" "$@"
    ...
    echo
    "$script_dir/specledger-executor.sh" "$change_dir"
    ;;
  status)
    if [[ $# -ne 1 ]]; then
      usage
  ...
  exec)
    ...
    "$script_dir/specledger-executor.sh" "$1"
    ;;
  approve)
    if [[ $# -ne 1 ]]; then
      usage
    ...
    "$script_dir/specledger-approve.sh" "$1"
    echo
    print_status "$1"
    echo
    "$script_dir/specledger-executor.sh" "$1" || true
    ;;
  next)
    if [[ $# -ne 1 ]]; then
      usage
    ...
    "$script_dir/specledger-continue.sh" "$1"
    echo
    print_status "$1"
    echo
    "$script_dir/specledger-executor.sh" "$1" || true
