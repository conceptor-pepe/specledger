...
usage() {
  cat <<'EOF'
Usage:
  scripts/specledger-continue.sh <change-dir>
EOF
}
...
if [[ "$waiting" == "true" ]]; then
  ...
  echo "Use specledger:approve or specld:approve first." >&2
  exit 1
fi
...
case "$current_stage" in
  implementation)
    ...
    ;;
  verify)
    "$script_dir/specledger-verify.sh" "$change_dir" >/dev/null
  ...
  commit-summary)
    ...
    ;;
  archive)
    "$script_dir/specledger-archive.sh" "$change_dir" >/dev/null
